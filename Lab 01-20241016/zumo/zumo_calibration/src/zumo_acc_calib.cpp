...
bool ZumoAccCalib::Init()
{
   ...
   //m_ZumoSensors_sub = mNH.subscribe("zumo/sensors_stamped", 100, &ZumoAccCalib::ZumoSensorCb, this);
   p_ZumoSubscription = this->create_subscription<zumo_msgs::msg::ZumoSensors>("/zumo/zumo_sensors", 100, std::bind(&ZumoAccCalib::ZumoSensorCb, this, _1)); 
   ...
   // serial read thread
   p_KeyboardThread = new boost::thread(boost::bind(&ZumoAccCalib::KeyboardLoop, this));
...
///*********************************************************************************
void ZumoAccCalib::ZumoSensorCb(const zumo_msgs::msg::ZumoSensors::ConstSharedPtr pMsg)
{
   ...
   if(POSE_Z_UPWARDS == i_Pose)
   {
      ...
      SetMatrices(i_MeasCount, 0, 0, GRAVITATIONAL_ACC, pMsg->ax,
                                                        pMsg->ay,
                                                        pMsg->az);
   }
   ...
   else if(POSE_Y_DOWNWARDS == i_Pose)
   {
      ...
      SetMatrices(i_MeasCount + 3*MEAS_COUNT, 0, -GRAVITATIONAL_ACC, 0, pMsg->ax,
                                                                        pMsg->ay,
                                                                        pMsg->az);
   }
   ...
   else if(POSE_X_DOWNWARDS == i_Pose)
   {
      ...
      SetMatrices(i_MeasCount + 5*MEAS_COUNT, -GRAVITATIONAL_ACC, 0, 0, pMsg->ax,
                                                                        pMsg->ay,
                                                                        pMsg->az);
   }
...
void ZumoAccCalib::KeyboardLoop()
{
   ...
   b_DataCollected = true; // we assume that the sensor data flow is uninterrupted.
...
void ZumoAccCalib::Calibrate()
{
   ...
   Eigen::Matrix<double, 4, MEAS_COUNT*6> mTranspose = m_MeasurementMatrix.transpose();
   Eigen::Matrix<double, 4, 4> mTempMatrix = mTranspose*m_MeasurementMatrix;
   Eigen::Matrix<double, 4, MEAS_COUNT*6> mCalibrationMatrix_ = mTempMatrix.inverse()*mTranspose;
