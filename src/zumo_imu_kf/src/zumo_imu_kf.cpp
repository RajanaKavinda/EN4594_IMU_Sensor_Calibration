...
bool ZumoImuKF::Init()
{
    ...
    p_ZumoSubscription = this->create_subscription<zumo_msgs::msg::ZumoSensors>("/zumo/zumo_sensors", 100, std::bind(&ZumoImuKF::ZumoSensorCb, this, _1));
    ...
    {
        RCLCPP_INFO(this->get_logger(), "%s: %s", TAYLOR_NORM_PARAMETER, b_TaylorNormalization ? TRUE : FALSE);
        RCLCPP_INFO(this->get_logger(), "%s: %s", IMU_DEMO_PARAMETER, b_IMUDemo ? TRUE : FALSE);
        RCLCPP_INFO(this->get_logger(), "%s: %s", PRINT_IMU_VALUES_PARAMETER, b_PrintValues ? TRUE : FALSE);
    }
    ...
    m_AccCalib_matrix(0,0) = cali11; m_AccCalib_matrix(0,1) = cali12; m_AccCalib_matrix(0,2) = cali13;
    m_AccCalib_matrix(1,0) = cali21; m_AccCalib_matrix(1,1) = cali22; m_AccCalib_matrix(1,2) = cali23;
    m_AccCalib_matrix(2,0) = cali31; m_AccCalib_matrix(2,1) = cali32; m_AccCalib_matrix(2,2) = cali33;
    m_AccCalib_matrix(3,0) = cali41; m_AccCalib_matrix(3,1) = cali42; m_AccCalib_matrix(3,2) = cali43;

    m_GyroBody_vec.setZero();
    m_GyroEarth_vec.setZero();

    // Kalman Filter
    m_StateX_vec.setZero(); m_StateX_vec[0] = 1.0;
    m_StateY_vec.setZero(); m_StateY_vec[1] = 1.0;
    m_StateZ_vec.setZero(); m_StateZ_vec[2] = 1.0;

    m_StateXCov_matrix.setZero();
    m_StateXCov_matrix(0,0) = m_StateXCov_matrix(1,1) = m_StateXCov_matrix(2,2) = INF;
    m_StateYCov_matrix.setZero();
    m_StateYCov_matrix(0,0) = m_StateYCov_matrix(1,1) = m_StateYCov_matrix(2,2) = INF;
    m_StateZCov_matrix.setZero();
    m_StateZCov_matrix(0,0) = m_StateZCov_matrix(1,1) = m_StateZCov_matrix(2,2) = INF;
...
///*********************************************************************************
void ZumoImuKF::ZumoSensorCb(const zumo_msgs::msg::ZumoSensors::ConstSharedPtr pMsg)
{
...
///*********************************************************************************
void ZumoImuKF::TransformAccRawData(const zumo_msgs::msg::ZumoSensors::ConstSharedPtr pMsg)
{
    ...
    /// Roll and Pitch Calculation (Eq. 23 and 24)
    d_Roll = atan2(m_Acc_vec[1], m_Acc_vec[2]); 
    d_Pitch = atan2(-m_Acc_vec[0], sqrt(pow(m_Acc_vec[1], 2) + pow(m_Acc_vec[2], 2)));  
...
///*********************************************************************************
void ZumoImuKF::TransformGyroRawData(const zumo_msgs::msg::ZumoSensors::ConstSharedPtr pMsg)
{
    /// conversion of GYRO data
    m_GyroBody_vec[0] = GYRO_CONVERSION_FACTOR*M_PI*(pMsg->gx/(180.0*1000.0));
    m_GyroBody_vec[1] = GYRO_CONVERSION_FACTOR*M_PI*(pMsg->gy/(180.0*1000.0));
    m_GyroBody_vec[2] = GYRO_CONVERSION_FACTOR*M_PI*(pMsg->gz/(180.0*1000.0));
...
///*********************************************************************************
void ZumoImuKF::TransformMagRawData(const zumo_msgs::msg::ZumoSensors::ConstSharedPtr pMsg)
{
    ...
    d_Yaw = -atan2((mTempMag_vec[1]*cos(d_Roll) - mTempMag_vec[2]*sin(d_Roll)),(mTempMag_vec[0]*cos(d_Pitch) + mTempMag_vec[1]*sin(d_Pitch)*sin(d_Roll) + mTempMag_vec[2]*sin(d_Pitch)*cos(d_Roll)));
...
{
   m_MeasRot_matrix(0,0) = cos(d_Yaw)*cos(d_Pitch); m_MeasRot_matrix(0,1) = -sin(d_Yaw)*cos(d_Roll) + cos(d_Yaw)*sin(d_Pitch)*sin(d_Roll); m_MeasRot_matrix(0,2) = sin(d_Yaw)*sin(d_Roll) + cos(d_Yaw)*sin(d_Pitch)*cos(d_Roll);
   m_MeasRot_matrix(1,0) = sin(d_Yaw)*cos(d_Pitch); m_MeasRot_matrix(1,1) = cos(d_Yaw)*cos(d_Roll) + sin(d_Yaw)*sin(d_Pitch)*sin(d_Roll);  m_MeasRot_matrix(1,2) = -cos(d_Yaw)*sin(d_Roll) + sin(d_Yaw)*sin(d_Pitch)*cos(d_Roll);
   m_MeasRot_matrix(2,0) = -sin(d_Pitch);           m_MeasRot_matrix(2,1) = cos(d_Pitch)*sin(d_Roll);                                      m_MeasRot_matrix(2,2) = cos(d_Pitch)*cos(d_Roll);
}
...
void ZumoImuKF::KalmanPredict()
{
    ...
    m_StateTransit_matrix(0,1) = -m_GyroEarth_vec[2]*dt;
    m_StateTransit_matrix(0,2) =  m_GyroEarth_vec[1]*dt;
    m_StateTransit_matrix(1,0) =  m_GyroEarth_vec[2]*dt;
    m_StateTransit_matrix(1,1) =  1;
    m_StateTransit_matrix(1,2) =  -m_GyroEarth_vec[0]*dt;
    m_StateTransit_matrix(2,0) =  -m_GyroEarth_vec[1]*dt;
    m_StateTransit_matrix(2,1) =  m_GyroEarth_vec[0]*dt;
    ...
    // eq. (5)
    // m_StateXCov_matrix, m_StateYCov_matrix, m_StateZCov_matrix will be updated at each operation
    // Rt is the process noise covariance matrix available as m_ProcessNoiseCov_matrix
    // Transpose of a matrix can be achieved using .transpose() function
    m_StateXCov_matrix = m_StateTransit_matrix * m_StateXCov_matrix * m_StateTransit_matrix.transpose() + m_ProcessNoiseCov_matrix;
    m_StateYCov_matrix = m_StateTransit_matrix * m_StateYCov_matrix * m_StateTransit_matrix.transpose() + m_ProcessNoiseCov_matrix;
    m_StateZCov_matrix = m_StateTransit_matrix * m_StateZCov_matrix * m_StateTransit_matrix.transpose() + m_ProcessNoiseCov_matrix;
...
void ZumoImuKF::Normalize()
{
    ...
    // dot product: x.dot(y)
    dError = mRotX_vec.transpose().dot(mRotY_vec);
    ...
    /// eq. (34) and (35)
    mRotXOrtho_vec = mRotX_vec - dError*mRotY_vec/2;
    mRotYOrtho_vec = mRotY_vec - dError*mRotX_vec/2;
    ...
    /// eq. (38), (39) and (40)
    {
        mRotXNorm_vec = 0.5*(3-mRotXOrtho_vec.dot(mRotXOrtho_vec))*mRotXOrtho_vec;
        mRotYNorm_vec = 0.5*(3-mRotYOrtho_vec.dot(mRotYOrtho_vec))*mRotYOrtho_vec;
        mRotZNorm_vec = 0.5*(3-mRotZOrtho_vec.dot(mRotZOrtho_vec))*mRotZOrtho_vec;
    }
    else
    /// eq. (37)
    // Help:
    ...
    {
        mRotXNorm_vec =  mRotXNorm_vec.normalized();
        mRotYNorm_vec =  mRotYNorm_vec.normalized();
        mRotZNorm_vec =  mRotZNorm_vec.normalized();
    }
...
void ZumoImuKF::KalmanUpdate()
{
    ...
    // 1) Innovations
    Eigen::Matrix<double, 3, 1>  mInnovationX_vec, mInnovationY_vec, mInnovationZ_vec;
    Eigen::Matrix<double, 3, 1>  mObservationX_vec, mObservationY_vec, mObservationZ_vec;
    ...
    // eq. (6) for all three: mInnovationX_vec, mInnovationY_vec and mInnovationZ_vec
    // C matrix is available as m_Obs_matrix
    ...
    // Qt matrix is the measurement noise covariance available as m_MeasNoiseCov_matrix
    mInnovCovX = m_Obs_matrix*m_StateXCov_matrix*m_Obs_matrix.transpose() + m_MeasNoiseCov_matrix;
    mInnovCovY = m_Obs_matrix*m_StateYCov_matrix*m_Obs_matrix.transpose() + m_MeasNoiseCov_matrix;
    mInnovCovZ = m_Obs_matrix*m_StateZCov_matrix*m_Obs_matrix.transpose() + m_MeasNoiseCov_matrix;
    ...
    mKalmanGainX = m_StateXCov_matrix*m_Obs_matrix.transpose()*mInnovCovX.inverse();
    mKalmanGainY = m_StateYCov_matrix*m_Obs_matrix.transpose()*mInnovCovY.inverse();
    mKalmanGainZ = m_StateZCov_matrix*m_Obs_matrix.transpose()*mInnovCovZ.inverse();
    ...
    // std::cout<<"m_StateY_vec[2] "<< m_StateY_vec[2]<<std::endl;
    // std::cout<<"m_StateZ_vec[2] "<< m_StateZ_vec[2]<<std::endl;

    // 5) Updated State Covariance
    // eq. (10) for all three: m_StateXCov_matrix, m_StateYCov_matrix and m_StateZCov_matrix
    // Help:
    // a 3x3 identity matrix is available as m_Identity_matrix
    m_StateXCov_matrix = (m_Identity_matrix - mKalmanGainX*m_Obs_matrix)*m_StateXCov_matrix;
    m_StateYCov_matrix = (m_Identity_matrix - mKalmanGainY*m_Obs_matrix)*m_StateYCov_matrix;
    m_StateZCov_matrix = (m_Identity_matrix - mKalmanGainZ*m_Obs_matrix)*m_StateZCov_matrix;
...
void ZumoImuKF::PublishTF()
{
    ...
    if(true == b_PrintValues)
    {
        printf("roll=%f pitch=%f yaw=%f\n\n", dRoll*180.0/M_PI, dPitch*180.0/M_PI, dYaw*180.0/M_PI);
    }
    ...
    tf2::Matrix3x3 mMat2;
    mMat2.setValue(m_MeasRot_matrix(0,0), m_MeasRot_matrix(0,1), m_MeasRot_matrix(0,2),
